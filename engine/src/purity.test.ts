...
describe('package purity', () => {
  ...
  test('every src/**/*.ts file only imports amtp-protocol, zod, node:* builtins, or in-package relatives', () => {
  ...
  test('isAllowedSpecifier rejects package-escaping relative specifiers', () => {
    ...
    expect(isAllowedSpecifier('./ports', 'foo.ts')).toBe(true)
  })

  test('isAllowedSpecifier resolves subdirectory-relative specifiers against the IMPORTING file, not the package root', () => {
    // src/testing/fakes.ts importing '../ports' resolves to src/ports.ts —
    // still inside the package even though the specifier string itself
    // starts with '..'.
    expect(isAllowedSpecifier('../ports', 'testing/fakes.ts')).toBe(true)
    // '../../shared/src/x' from testing/evil.ts resolves to shared/src/x —
    // one level above the package root, so it must be rejected.
    expect(isAllowedSpecifier('../../shared/src/x', 'testing/evil.ts')).toBe(false)
    // './../../x' from testing/evil.ts normalizes the same way — reaches
    // above the package root and must be rejected too.
    expect(isAllowedSpecifier('./../../x', 'testing/evil.ts')).toBe(false)
  })
  ...
  test('isAllowedSpecifier accepts amtp-protocol but no other bare package', () => {
